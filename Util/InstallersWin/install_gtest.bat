...
:arg-parse  :: 这是一个标签，用于goto命令的跳转，以实现命令行参数的循环解析。

if not "%1"=="" (  :: 检查第一个参数（%1）是否存在且不为空。
    :: 如果参数是"--build-dir"，则执行以下操作：
    if "%1"=="--build-dir" (
        :: 注意：这里有一个错误。%~dpn2 应该是 %2 的盘符、路径和文件名（不包括扩展名）。
        :: 但对于 "--build-dir" 参数，我们期望的是一个路径作为值，所以应该使用 %~2（如果要去除引号）或 "%2"（如果保留引号）。
        :: 然而，%~2 仍然不是正确的选择，因为它会去除路径中的引号并可能导致问题。
        :: 正确的做法是直接使用 "%2" 并确保调用脚本时提供了有效的路径。
        set BUILD_DIR=%~2  :: 错误：应该使用 "%2" 来确保路径的完整性（包括引号）。
                            :: 但要注意，这样做可能会保留引号，这取决于如何传递参数。
                            :: 如果确保路径中没有空格或特殊字符，可以使用 %2（不带引号）。
        :: 为了简洁和安全，这里我们假设使用 "%2" 并手动处理引号（如果需要）。
        :: 注意：下面的示例代码没有处理引号，因为它取决于具体的使用场景。
        :: set "BUILD_DIR=%~2" 并通过其他方式处理可能的引号（例如使用for循环）。
        
        :: 由于上述原因，这里直接改用 "%2" 并提醒用户确保路径正确。
        set "BUILD_DIR=%2"  :: 设置BUILD_DIR环境变量为"--build-dir"参数后的路径。
        shift  :: 将参数列表左移一位，即原来的第二个参数现在成为新的第一个参数。
    )
    
    :: 如果参数是"--generator"，则执行以下操作：
    if "%1"=="--generator" (
        :: 设置GENERATOR环境变量为"--generator"参数后的值（即第三个命令行参数，因为%2已被shift掉）。
        set "GENERATOR=%2"  :: 注意：这里%2是shift之后的第一个参数，即原来的第三个参数。
        shift  :: 再次将参数列表左移一位。
    )
    
    :: 如果参数是"--delete-src"，则执行以下操作：
    if "%1"=="--delete-src" (
        :: 设置DEL_SRC环境变量为true，表示需要删除源文件。
        set DEL_SRC=true
    )
    
    :: 无论是否处理了当前参数，都使用shift命令将参数列表左移一位。
    shift
    
    :: 使用goto命令跳转到标签:arg-parse的位置，继续循环解析剩余的参数。
    goto :arg-parse
)
