...
//输出批处理脚本接收到的参数（主要用于调试目的，%*表示获取传递给批处理脚本的所有参数），并在前面加上之前定义的FILE_N标识，便于区分不同脚本输出的信息。

rem ============================================================================
rem -- Parse arguments ---------------------------------------------------------
rem ============================================================================
//以下是一个大的注释块，用于标识下面的代码是用于解析传入批处理脚本的参数的功能部分，使代码结构更清晰。
...
if not "%1"=="" (
    if "%1"=="--build-dir" (
        set BUILD_DIR=%~dpn2
        shift
//如果传入的第一个参数是"--build-dir"，则设置BUILD_DIR变量的值为传入的第二个参数所表示的路径（%~dpn2表示获取第二个参数的驱动器、路径和文件名部分）， 然后使用shift命令将参数列表向左移动一位，去掉已经处理过的参数，以便继续处理下一个参数。
    )
    )
    if "%1"=="-h" (
        goto help
