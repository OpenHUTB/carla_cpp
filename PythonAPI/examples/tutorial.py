...
try:
    # 首先，我们需要创建一个客户端（client），它将用于向模拟器发送请求。
    # 这里我们假设模拟器在本地主机（localhost）的2000端口接受请求。
    client = carla.Client('localhost', 2000)
    # 设置客户端操作的超时时间为2秒，避免长时间等待无响应的情况。
    client.set_timeout(2.0)

    # 通过已创建的客户端获取当前正在运行的世界（world）对象，
    # 这个世界对象包含了模拟场景中的各种元素和相关信息。
    world = client.get_world()

    # 从世界对象中获取蓝图库（blueprint_library），蓝图库中包含了可用于在模拟场景中添加新“演员”（actors，例如车辆、行人等实体）的各种蓝图定义。
    blueprint_library = world.get_blueprint_library()

    # 接下来，从蓝图库中筛选出类型为'vehicle'（车辆）的所有蓝图，并随机选择其中一个蓝图。
    # 这个蓝图将用于后续创建具体的车辆实例。
    bp = random.choice(blueprint_library.filter('vehicle'))

    # 一个蓝图包含了定义车辆实例的一系列属性列表，我们可以读取这些属性并且修改其中的一部分属性。
    # 例如，在这里我们随机设置车辆的颜色，前提是该蓝图具有'color'这个属性可设置。
    if bp.has_attribute('color'):
        # 从蓝图的'color'属性推荐值列表中随机选择一个颜色值。
        color = random.choice(bp.get_attribute('color').recommended_values)
        # 将所选的颜色值设置给蓝图的'color'属性，这样后续基于该蓝图创建的车辆就会应用这个随机颜色。
        bp.set_attribute('color', color)

    # 现在我们需要为车辆指定一个初始的变换（transform）信息，也就是车辆在地图中的初始位置和姿态等信息。
    # 这里我们从地图推荐的生成点（spawn points）列表中随机选择一个作为车辆的初始位置和姿态。
    transform = random.choice(world.get_map().get_spawn_points())

    # 告知世界对象按照给定的蓝图（bp）和初始变换（transform）信息来生成车辆实例，也就是在模拟场景中创建出实际的车辆。
    vehicle = world.spawn_actor(bp, transform)

    # 需要重点注意的是，我们创建的这些“演员”（像这里的车辆）在模拟场景中不会自动被销毁，
    # 除非我们显式地调用它们的“destroy”函数来销毁它们。
    # 如果我们没有调用“destroy”函数，那么即使我们退出了Python脚本，这些“演员”依然会留在模拟场景中。
    # 基于这个原因，我们将创建的所有“演员”都存储到一个列表（actor_list）中，以便后续能够统一销毁它们。
    actor_list.append(vehicle)
    print('created %s' % vehicle.type_id)

    # 让创建的车辆开启自动驾驶模式，使其能够在模拟场景中自动行驶。
    vehicle.set_autopilot(True)

    # 接下来我们要添加一个“深度”相机，并将其附着到车辆上。需要注意的是，
    # 这里给相机设置的变换（transform）信息是相对于车辆的，即基于车辆的坐标系来确定相机的位置和姿态等信息。
    # （此处代码后续应该还有关于相机创建及图像保存相关的部分，但这里暂未完整给出）
