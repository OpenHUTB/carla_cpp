...
class HelpText(object):
    ...
    这个类是一个辅助类，用于借助 `pygame` 库来处理文本输出相关操作。它的主要功能是创建包含帮助文本信息的可视化表面（Surface），
    并能够控制该文本在给定显示界面上的显示与隐藏，方便在程序运行过程中为用户展示操作帮助等相关文字内容。
    """
    def __init__(self, font, width, height):
        """
        类的构造函数，用于初始化 `HelpText` 类实例的各项属性，为后续文本显示相关操作做准备。

        参数说明：
        - `font`：一个 `pygame.font.Font` 类型的对象，代表了用于渲染文本的字体样式。通过这个字体对象，可以将文本字符串转化为在界面上可显示的图像形式（即文本纹理），确定文本呈现出来的字体外观。
        - `width`：一个数值，表示整个显示界面在水平方向上的宽度，单位通常是像素。这个参数用于后续计算帮助文本显示区域在显示界面中的水平位置，以实现合理的布局。
        - `height`：一个数值，表示整个显示界面在垂直方向上的高度，单位同样通常是像素。用于计算帮助文本显示区域在显示界面中的垂直位置，确保文本显示在合适的位置上。
        """
        lines = __doc__.split('\n')
        # 将类的文档字符串（`__doc__`，也就是类定义上方用三引号包裹的描述性文字内容）按照换行符 `\n` 进行分割，得到一个字符串列表 `lines`。
        # 这里默认将类的文档字符串作为帮助文本的内容来源，不过在实际应用中，也可以根据具体需求替换为其他更详细、具体的帮助文本内容。

        self.font = font
        # 将传入的字体对象赋值给实例的 `self.font` 属性，方便后续在渲染文本时使用该指定字体，确保文本以期望的字体样式显示。

        self.line_space = 18
        # 初始化实例属性 `self.line_space`，设置其值为 `18`，这个值代表每行帮助文本之间在垂直方向上的间距，单位通常是像素。
        # 通过设置合适的行间距，可以使帮助文本在显示时排版更加清晰、美观，便于用户阅读。

        self.dim = (780, len(lines) * self.line_space + 12)
        # 初始化实例属性 `self.dim`，它是一个表示二维尺寸的元组，用于确定帮助文本显示区域的大小。
        # 其中宽度固定设置为 `780` 像素，高度则根据帮助文本的行数（通过 `len(lines)` 获取）乘以每行的间距（`self.line_space`）再加上 `12` 像素来计算得出。
        # 这个额外加上的 `12` 像素可能是作为上下边距之类的预留空间，使得整个帮助文本显示区域在垂直方向上有更合适的布局。

        self.pos = (0.5 * width - 0.5 * self.dim[0], 0.5 * height - 0.5 * self.dim[1])
        # 初始化实例属性 `self.pos`，它同样是一个元组，用于确定帮助文本显示区域在整个显示界面中的位置坐标（水平坐标和垂直坐标）。
        # 通过将显示界面宽度和高度的一半分别减去帮助文本显示区域尺寸的一半，实现将帮助文本显示区域定位在显示界面的中心位置，达到居中显示的效果，让帮助文本在界面上呈现得更加规整、美观。

        self.seconds_left = 0
        # 初始化实例属性 `self.seconds_left` 为 `0`。从变量名推测，这个属性原本可能是用于记录帮助文本剩余显示的时间（单位可能是秒），
        # 不过在当前给出的代码逻辑中，暂时没有看到基于该属性对帮助文本显示时长进行控制的相关操作，也许是预留的功能，有待后续进一步完善相关逻辑。

        self.surface = pygame.Surface(self.dim)
        # 创建一个新的 `pygame.Surface` 对象，这个对象代表了一个二维的图像表面，其大小由前面初始化的 `self.dim` 属性确定。
        # 这个表面将作为承载帮助文本内容的载体，后续所有与帮助文本绘制相关的操作都将在这个表面上进行，例如绘制文本图像等。

        self.surface.fill((0, 0, 0, 0))
        # 使用特定的颜色来填充这个新创建的表面，这里传入的颜色参数 `(0, 0, 0, 0)` 表示黑色且完全透明的颜色。
        # 这么做的目的是先将表面清空或者说设置为初始的透明状态，方便后续在上面进行绘制文本等操作，使表面初始状态符合预期的显示效果设定。

        for n, line in enumerate(lines):
            text_texture = self.font.render(line, True, (255, 255, 255))
            self.surface.blit(text_texture, (22, n * self.line_space))
            self._render = False
            # 对分割后的帮助文本行列表（`lines`）进行遍历，针对每一行文本执行以下操作：
            # - 首先，使用实例的字体对象（`self.font`）调用 `render` 方法，将当前行的文本内容（`line`）渲染成一个图像表面（`text_texture`），
            # 并设置文本颜色为白色（`(255, 255, 255)`），使得文本以白色的样式呈现为可视化的图像形式，便于后续在表面上进行绘制展示。
            # - 接着，使用 `self.surface`（也就是前面创建的用于承载文本的那个表面）的 `blit` 方法，将渲染好的文本图像表面（`text_texture`）绘制到 `self.surface` 上。
            # 绘制的位置坐标在水平方向为 `22` 像素（可能是为了预留一定的左边距，使文本在显示区域内看起来更加协调、美观），垂直方向则是当前行的索引值（`n`）乘以每行的间距（`self.line_space`），
            # 通过这样的方式逐行将帮助文本绘制到指定的表面上，完成文本内容的排版布局。
            # - 最后，将实例属性 `_render` 设置为 `False`，从后续代码逻辑以及变量名来看，这个属性是用于控制帮助文本是否要渲染显示到最终的显示界面上的，初始设置为 `False` 表示默认情况下帮助文本是隐藏状态，不进行显示。

        self.surface.set_alpha(220)
        # 设置帮助文本所在表面（`self.surface`）的透明度（alpha 值）为 `220`。通过设置透明度，使得这个表面呈现出一定的半透明效果，
        # 这样在将帮助文本显示到最终界面上时，既能让用户清晰地看到文本内容，又不会完全遮挡住后面的界面元素，从而营造出较好的视觉显示效果。

    def toggle(self):
        """
        函数功能：用于切换帮助文本的显示与隐藏状态。通过对实例属性 `_render` 进行取反操作来实现这个切换功能，
        方便用户在程序运行过程中，通过特定的操作（比如按下某个对应的按键等）来控制帮助信息是否在界面上显示出来。
        """
        self._render = not self._render
        # 将实例属性 `_render` 的值进行取反。如果原来 `_render` 的值为 `True`，表示帮助文本当前处于显示状态，取反后变为 `False`，则帮助文本将被隐藏；
        # 反之，如果原来 `_render` 为 `False`，取反后变为 `True`，帮助文本就会显示出来，以此实现显示状态的切换功能。

    def render(self, display):
        """
        函数功能：根据帮助文本当前的显示状态（由实例属性 `_render` 决定），将包含帮助文本的表面（`self.surface`）渲染到给定的显示界面（`display`）上。
        如果 `_render` 属性为 `True`，则执行绘制操作，把帮助文本显示在界面上；若 `_render` 为 `False`，则不进行绘制，帮助文本保持隐藏状态。

        参数说明：
        - `display`：一个 `pygame` 中的显示对象，它代表了整个程序用于展示各种元素的界面，所有的可视化元素最终都需要通过相应的绘制操作渲染到这个对象所代表的界面上，
        这里就是将帮助文本所在的表面绘制到这个显示界面上，以呈现给用户查看帮助信息内容。
        """
        if self._render:
            display.blit(self.surface, self.pos)
            # 判断实例属性 `_render` 的值是否为 `True`，如果是，则使用 `display`（显示界面对象）的 `blit` 方法，将帮助文本所在的表面（`self.surface`）绘制到显示界面上。
            # 绘制的位置是之前在初始化时确定好的坐标位置（`self.pos`），通过这样的操作，帮助文本就会按照预定的位置和样式显示在界面上，供用户查看相关的帮助信息。
