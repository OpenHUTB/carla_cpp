    ...
    void load_obj(const char *filename, bool process_uv = false)
    {
     ...
     // 解析材质库
      ...
      // 解析纹理坐标
        ...
        // 如果所有尝试的格式都无法匹配当前读取的面信息行，说明遇到了无法识别的格式，
        // 则输出相应的错误提示信息到控制台，告知用户出现了无法识别的序列，并输出当前行的内容，
        // 然后进入一个无限循环（这里只是示例代码逻辑，实际应用中可能需要更合理的错误处理方式），
        // 因为当前代码逻辑不知道如何继续处理这种错误情况，所以暂停在这里等待进一步处理（比如手动终止程序等）。
            else
            {
