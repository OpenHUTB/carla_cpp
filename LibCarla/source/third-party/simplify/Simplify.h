     ...
     // 解析材质库
      ...
      // 解析纹理坐标
        ...
        if (line[0] == 'f')
        {
          ...
          if (tri_ok)
          { 
             // 对于Triangle结构体（或类）中的v数组（应该是用于存储构成三角形面片的顶点索引的成员变量），
             // 将从文件中读取并解析得到的顶点索引值减1再减去vertex_cnt后赋值给相应的元素，
             // 减1可能是因为在.obj文件中顶点索引通常是从1开始计数，而在程序内部的数组等数据结构中通常是从0开始计数，
             // 减去vertex_cnt可能是为了处理在文件中可能存在多个对象等情况时，对顶点索引进行相对偏移等操作，使其对应到正确的顶点位置。
