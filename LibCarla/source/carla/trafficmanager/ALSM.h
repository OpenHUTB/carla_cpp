...
using ActorList = carla::SharedPtr<cc::ActorList>; // 定义演员列表共享指针类型
using ActorMap = std::unordered_map<ActorId, ActorPtr>; // 定义演员映射表类型
...
private:
  AtomicActorSet &registered_vehicles; // 引用已注册车辆的原子集合
  ActorMap unregistered_actors; // 存储未注册车辆的结构
  BufferMap &buffer_map; // 引用缓冲区映射
  IdleTimeMap idle_time; // 存储车辆在位置上停留时间的结构
  ActorMap hero_actors; // 存储角色名称为"hero"的车辆
  TrackTraffic &track_traffic; // 引用交通跟踪对象
  std::vector<ActorId>& marked_for_removal; // 标记待移除车辆的数组
  ...
  double elapsed_last_actor_destruction {0.0}; // 记录自上次因闲置过久而销毁车辆的时间
  cc::Timestamp current_timestamp; // 当前时间戳
  std::unordered_map<ActorId, bool> has_physics_enabled; // 存储每个演员是否启用物理的映射
  ...
  using DestroyeddActors = std::pair<ActorIdSet, ActorIdSet>; // 定义删除演员的数据类型
  // 确定在上一帧中删除的演员
  // 返回已注册和未注册演员的数组
